/*
    ...
    Copyright (C) 2004, 2005, 2008, 2012, 2019, 2026 Rocky Bernstein
   <rocky@gnu.org> adapted from cuetools Copyright (C) 2003 Svend Sanjay
   Sorensen <ssorensen@fastmail.fm>
...
typedef enum {
  ...
  CDTEXT_FIELD_MESSAGE =
      4, /**< message(s) from content provider or artist, ISO-8859-1 encoded*/
  CDTEXT_FIELD_ARRANGER = 5, /**< name(s) of the arranger(s) */
  CDTEXT_FIELD_ISRC = 6,     /**< ISRC code of each track */
  CDTEXT_FIELD_UPC_EAN =
      7, /**< upc/european article number of disc, ISO-8859-1 encoded */
  CDTEXT_FIELD_GENRE =
      8, /**< genre identification and genre information, ASCII encoded */
  CDTEXT_FIELD_DISCID =
      9, /**< disc identification, ASCII encoded (may be non-printable) */
...
/**
  ...
  Each sets the buffer passed in to values that begin with a 4-byte header. This
  should be skipped. Here is some sample code:
...
/**
  ...
  `NULL` is returned if the key is #CDTEXT_FIELD_INVALID or the field is not
  set. Strings are encoded in UTF-8.
...
/**
  ...
  If an enumeration is #CDTEXT_LANGUAGE_INVALID, then the language block has an
 ...
 */
void cdtext_set(cdtext_t *p_cdtext, cdtext_field_t key, const uint8_t *value,
